...
const VERTEX_SIZE: usize = VERTEX_POS_SIZE
    + VERTEX_NORMAL_SIZE
    + VERTEX_TANGENT_SIZE
...
impl GameWindow {
    ...
    pub fn new(width: u32, height: u32, title: String) -> Result<GameWindow, String> {
        ...
        let gl_window = try!(gl_window_builder
            .build()
            .map_err(|_| creation_err.to_string()));
        unsafe {
            try!(gl_window
                .make_current()
                .map_err(|_| creation_err.to_string()))
    ...
    fn bind_vao_checked(&mut self, vao: GLuint) {
        unsafe {
    ...
    fn initialize_vbo(&mut self, max: usize) {
        ...
        unsafe {
            ...
            gl::BufferData(
                gl::ARRAY_BUFFER,
                float_size!(buffer_size, GLsizeiptr),
                0 as CVoid,
                gl::STATIC_DRAW,
            );
        }
        self.vbos.push((vbo, 0, buffer_size));
        let size = if self.vaos.is_empty() {
            0
        } else {
            self.vaos[0].len()
        };
    ...
    fn initialize_ebo(&mut self, max: usize) {
        let buffer_size = cmp::max(max, BUFFER_SIZE);
        let working_vao = self.working_vao.clone();
        self.bind_vao_checked(working_vao);
        let mut ebo = 0;
        unsafe {
            ...
            gl::BufferData(
                ...
                uint_size!(buffer_size, GLsizeiptr),
                0 as CVoid,
                gl::STATIC_DRAW,
            );
        }
    ...
    fn initialize_vao(&mut self, vbo: usize, ebo: usize) -> GLuint {
        unsafe {
            match self.vaos[vbo as usize][ebo as usize] {
                ...
                None => {
                    ...
                    gl::VertexAttribPointer(
                        pos_attr as GLuint,
                        VERTEX_POS_SIZE as i32,
                        gl::FLOAT,
                        gl::FALSE as GLboolean,
                        float_size!(VERTEX_SIZE, GLsizei),
                        ptr::null(),
                    );
                    ...
                    gl::VertexAttribPointer(
                        tangent_attr as GLuint,
                        VERTEX_TANGENT_SIZE as i32,
                        gl::FLOAT,
                        gl::FALSE as GLboolean,
                        float_size!(VERTEX_SIZE, GLsizei),
                    ...
                    gl::VertexAttribPointer(
                        bitangent_attr as GLuint,
                        VERTEX_BITANGENT_SIZE as i32,
                        gl::FLOAT,
                        gl::FALSE as GLboolean,
                        float_size!(VERTEX_SIZE, GLsizei),
                        float_size!(
                            VERTEX_POS_SIZE + VERTEX_NORMAL_SIZE + VERTEX_TANGENT_SIZE,
                            CVoid
                        ),
                    );
                    ...
                    gl::VertexAttribPointer(
                        tcoord_attr as GLuint,
                        VERTEX_TCOORD_SIZE as i32,
                        gl::FLOAT,
                        gl::FALSE as GLboolean,
                        float_size!(VERTEX_SIZE, GLsizei),
                        float_size!(
    ...
    pub fn set_gamma(&mut self, gamma: GLfloat) {
        self.gamma = gamma;
        unsafe {
    ...
    pub fn update_point_light(&self, index: usize) {
    ...
    pub fn remove_point_light(&mut self, index: usize) -> light::PointLight {
        ...
        unsafe {
            uniform_uint!(self.program, lights![free_index, "type"], 0);
        };
    ...
    pub fn update_directional_light(&self, index: usize) {
    ...
    // field changes for the changes to appear in-world.
    pub fn update_spot_light(&self, index: usize) {
    ...
    pub fn map_vbo(&mut self, info: Rc<model::ModelInfo>) {
        ...
        unsafe {
            gl::BufferSubData(
                gl::ELEMENT_ARRAY_BUFFER,
                uint_size!(ebo_pair.1, GLintptr),
                uint_size!(elements.len(), GLsizeiptr),
                vec_to_addr!(elements),
            );
            gl::BufferSubData(
                gl::ARRAY_BUFFER,
                float_size!(vbo_pair.1, GLintptr),
                float_size!(vertices.len(), GLsizeiptr),
                vec_to_addr!(vertices),
            );
        }
    ...
    pub fn draw_instance(&mut self, instance: &model::ModelInstance) {
        ...
        unsafe {
            ...
            let diffuse_id = if mat.diffuse == 0 {
            ...
            match mat.normal {
                ...
                None => {
                    uniform_int!(self.program, "use_normal_map", 0);
                }
            };
