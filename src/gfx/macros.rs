...
macro_rules! uniform_mat4 {
    ($p:expr, $s:expr, $l: expr) => {
        gl::UniformMatrix4fv(
            gl::GetUniformLocation($p, gl_str!($s)),
            1,
...
macro_rules! uniform_vec3 {
    ($p:expr, $s:expr, $l: expr) => {
        gl::Uniform3fv(gl::GetUniformLocation($p, gl_str!($s)), 1, ($l).as_ptr())
    };
}
...
macro_rules! uniform_vec4 {
    ($p:expr, $s:expr, $l: expr) => {
        gl::Uniform4fv(gl::GetUniformLocation($p, gl_str!($s)), 1, ($l).as_ptr())
    };
}
